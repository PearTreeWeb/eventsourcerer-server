...
#[AsMessageHandler]
final readonly class ResetProjectionHandler
{
    ...
    public function __invoke(ResetProjection $command): void
    {
        $projection = $this->projectionRepository->find($command->id);
        $projection = $this->projectionRepository->resetProjection($projection, $this->clock->now());
        ...
        foreach ($this->streamEventRepository->allIterableRaw($projection->getStartDate(), $projection->getEndDate()) as $streamEvent) {
            ...
            foreach ($mutations as $mutation) {
                ...
                $masterState = $pendingStates[$masterKey]
                    ?? $this->projectionStateRepository->findResetByProjectionId($command->id)
                    ?? ProjectionState::create($command->id, ProjectionStateType::Reset, $this->clock->now());
                ...
                $partitionedState = $pendingStates[$partitionedKey]
                    ?? $this->projectionStateRepository->findByStreamAndProjectionId($streamId, ProjectionStateType::Reset, $command->id)
                    ?? ProjectionState::create($command->id, ProjectionStateType::Reset, $this->clock->now(), $streamId);
                ...
                if (null !== $updatedMasterState || null !== $updatedPartitionedState) {
                    $lastAllSequence = (int) $streamEvent['all_sequence'];
                }
            }
        ...
        foreach ($this->projectionStateRepository->findAllCurrentWithProjectionId($projection->getId()) as $existing) {
            $this->projectionStateRepository->delete($existing);
        }

        foreach ($this->projectionStateRepository->findAllResetStatesWithProjectionId($projection->getId()) as $reset) {
            $reset->setType(ProjectionStateType::Main);

            $this->projectionStateRepository->update($reset);
        }
